    ...
    **YOU NEED TO IMPLEMENT FUNCTIONS IN THIS CLASS**
    ...
    def act(self, obs: np.ndarray, explore: bool):
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q3**
        ...
        if not explore:
            ...
            # in case of ties we want to randomly select, if there is no tie we just select the max
            max_actions = (q_values == max_value).nonzero(as_tuple=True)[0]
            action = np.random.choice(max_actions.tolist())
        else:
            # epsilon greedy
    ...
    def update(self, batch: Transition) -> Dict[str, float]:
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q3**
        ...
        # gather method selects the q-value for the action in actions, and for the state in the bathc based on the position of the action in actions
        q_values = q_values.gather(1, actions)

        # Calculate target Q-values
        with torch.no_grad():
            # Get max Q-value for next state from target network
            next_q_values = self.critics_target(next_states).max(1, keepdim =True)[0] # gets max for each sample in batch, [0] returns the values only
